...
void bank1() {  //LOOP STATION BANK
  ...
  buttleft.poll();
  if (buttleft.longPress()) {  //RECORD
    MIDI.sendControlChange(23, 127, 1);
  }
  if (buttleft.singleClick()) {  //CLEAR
...
void bank4() { //PLAYBACK bank

  buttright.poll();
  if (buttright.longPress()) {

    MIDI.sendControlChange(102, 127, 1);
  }
  if (buttright.doubleClick()) { 
    MIDI.sendControlChange(103, 127, 1);
  }

  if (buttright.singleClick()) { 
    MIDI.sendControlChange(104, 127, 1);
  }

  buttleft.poll();
  if (buttleft.longPress()) {
    MIDI.sendControlChange(105, 127, 1);
  }

  if (buttleft.doubleClick()) {
...
void bank5() {
  ...
  if (buttright.longPress()) {
    ...
    MIDI.sendControlChange(108, 127, 1);
  }
  if (buttright.doubleClick()) { 
    MIDI.sendControlChange(109, 127, 1);
  }

  if (buttright.singleClick()) { 
    MIDI.sendControlChange(110, 127, 1);
  }
...
void bank6() {
  ...
  if (buttright.singleClick()) { 
    MIDI.sendControlChange(116, 127, 1);
  }

  buttleft.poll();
  if (buttleft.longPress()) {
    MIDI.sendControlChange(117, 127, 1);
  }
  ...
  if (buttleft.singleClick()) { 
    MIDI.sendControlChange(119, 127, 1);
  }
